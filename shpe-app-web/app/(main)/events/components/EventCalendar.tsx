    ...
    return (
        <div className="w-auto h-[800px] mx-3 mb-5 text-black rounded-2xl border-solid border-[#E0E0E0] border-[3px] flex flex-row flex-shrink-0">
            ...
            <div className="flex-grow flex flex-col">
                ...
                <div className="w-full h-16 flex flex-row items-center justify-around border-l-[3px] border-[#E0E0E0]">
                    ...
                    <div className="h-10 w-64">
                        <button 
                            ...
                            className={`${isMonthSelected ? 'w-3/5 bg-[#500000] text-white' : 'w-2/5 bg-[#e8e7e7] text-[#A8A8A8]'} transition-all duration-500 h-full font-semibold rounded-l-md`}>
                            Month
                        </button>
                        <button 
                            onClick={() => setIsMonthSelected(false)}
                            className={`${!isMonthSelected ? 'w-3/5 bg-[#500000] text-white' : 'w-2/5 bg-[#e8e7e7] text-[#A8A8A8]'} transition-all duration-500 h-full font-semibold rounded-r-md`}>
                            Week
                        </button>
                    </div>
                    <div className="h-14 w-32 gap-2 flex items-center">
                        <button onClick={() => setFocusDate(subMonths(focusDate, 1))} className="flex items-center justify-center h-10 w-16 bg-[#e8e7e7] rounded-xl border-solid border-[#E0E0E0] border-[3px]">
                            <img src="alt-arrow.svg" className="w-5/6 h-5/6"/>
                        </button>
                        <button onClick={() => setFocusDate(addMonths(focusDate, 1))} className="flex items-center justify-center h-10 w-16 bg-[#e8e7e7] rounded-xl border-solid border-[#E0E0E0] border-[3px]">
                ...
                {/* Main Calendar */}
                {   isMonthSelected ? 
                    <MonthView eventsByDate={eventsByDate} focusDate={focusDate}/> :
