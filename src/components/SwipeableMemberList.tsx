...
const SwipeableMemberCard = ({ userData, onSwipe }: { userData: PublicUserInfo; onSwipe: any }) => {
  ...
  return (
    ...
    >
      <View className="mx-5">
        <MemberCard userData={userData} displayPoints={true} />
      </View>

      <DismissibleModal visible={confirmVisible && invisibleConfirmModal} setVisible={setConfirmVisible}>
        <View
        ...
        >
          ...
          <Text className={`text-md ${darkMode ? 'text-white' : 'text-black'}`}>
            You will be setting {userData?.name} as the member of the month.
          </Text>
