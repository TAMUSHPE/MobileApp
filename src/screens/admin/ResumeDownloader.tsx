    ...
    <SafeAreaView className={`flex-1 ${darkMode ? 'bg-primary-bg-dark' : 'bg-primary-bg-light'}`}>
      ...
      <View className="space-y-4 mx-6 mt-10">
        ...
        <Text className={`${darkMode ? 'text-white' : 'text-black'}`}>
          This is a resume bundle downloader that downloads all resume that a user has uploaded{' '}
        </Text>

        <Text className={`${darkMode ? 'text-white' : 'text-black'}`}>
          Click the button below to generate a URL link. This URL link will contain a direct download for a zip file.
          This URL Link will be valid for only <Text className="font-bold">1 hour</Text>
        </Text>

        <Text className={`${darkMode ? 'text-white' : 'text-black'}`}>
          The generation of this URL link will vary and depends on the amount of resume being generated. This can take
          up to 20 minutes. <Text className="font-bold">You may leave the app and come back</Text>.
        </Text>

        <Text className={`mb-8 ${darkMode ? 'text-white' : 'text-black'}`}>
          When the link is generated confirm the creation time. You may either open the link or share it
        </Text>

        <Text className={`text-red-1 text-center text-bold`}>
          NEVER SHARE THIS LINK WITH ANYONE OTHER THAN YOUR OWN DEVICES OR AUTHORIZED PERSONNEL
        </Text>
