    ...
    <SafeAreaView className={`flex-1 ${darkMode ? 'bg-primary-bg-dark' : 'bg-primary-bg-light'}`} edges={['top']}>
      ...
      {members && members.length > 0 && (
        <View className="mt-9 flex-1">
          <Text className={`ml-6 text-xl font-semibold mb-5 ${darkMode ? 'text-white' : 'text-black'}`}>
            Select a user
          </Text>
          <MembersList
            handleCardPress={(uid) => {
      ...
      {loading && <ActivityIndicator size="small" className="mt-8" />}

      <DismissibleModal visible={confirmVisible} setVisible={setConfirmVisible}>
        <View
        ...
        >
          ...
          <TouchableOpacity
            ...
            onPress={async () => {
              handleLinkPress(selectedMemberDocuments?.resumePublicURL!);
            }}
          >
      ...
      <DismissibleModal visible={infoVisible} setVisible={setInfoVisible}>
        <View
          className={`flex opacity-100 rounded-md p-6 space-y-6 ${
        ...
        >
          <View className="flex-row items-center justify-between">
            <View className="flex-row items-center">
              ...
              <Text className={`text-2xl font-semibold ml-2 ${darkMode ? 'text-white' : 'text-black'}`}>
                Instructions
              </Text>
            </View>
            <View>
          ...
          <View className="w-[85%]">
            <Text className={`text-md font-semibold ${darkMode ? 'text-white' : 'text-black'}`}>
              Members that upload resume will appear here.
            </Text>
          </View>

          <View className="w-[85%]">
            <Text className={`text-md font-semibold ${darkMode ? 'text-white' : 'text-black'}`}>
              To begin verification, click on a member and view their Resume.
            </Text>
          </View>

          <View className="w-[85%]">
            <Text className={`text-md font-semibold ${darkMode ? 'text-white' : 'text-black'}`}>
              Keep in mind that this resume bank should only include “high quality” resume.
            </Text>
          </View>

          <View className="w-[85%]">
            <Text className={`text-md font-semibold ${darkMode ? 'text-white' : 'text-black'}`}>
              For the safety of our members, we advice denying resume that contain private information such as address.
            </Text>
          </View>

          <View className="w-[85%]">
            <Text className={`text-md font-semibold ${darkMode ? 'text-white' : 'text-black'}`}>
              Click Approve or Deny, the resume will appear on the resume bank, and the member will be notified.
            </Text>
          </View>
        </View>
      </DismissibleModal>
