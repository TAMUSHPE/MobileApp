...
const Home = ({ navigation, route }: NativeStackScreenProps<HomeStackParams>) => {
  ...
  const hasPrivileges =
    userInfo?.publicInfo?.roles?.admin?.valueOf() ||
    userInfo?.publicInfo?.roles?.officer?.valueOf() ||
    userInfo?.publicInfo?.roles?.developer?.valueOf();
  ...
  const InterestButtons = ({ interestEvent, label }: { interestEvent: EventType; label: string }) => {
    const isSelected = userInterests.includes(interestEvent);
    return (
      <TouchableOpacity
        onPress={() => handleInterestToggle(interestEvent)}
        className={`justify-center items-center flex-row border py-2 px-3 mr-6 mb-4 rounded-md ${
          isSelected
            ? 'border-primary-blue bg-primary-blue'
            : darkMode
            ? 'bg-secondary-bg-dark border-white'
            : 'bg-secondary-bg-light border-black'
        }`}
        style={{
          shadowColor: '#000',
          shadowOffset: {
    ...
    <SafeAreaView edges={['top']} className={`h-full ${darkMode ? 'bg-primary-bg-dark' : 'bg-primary-bg-light'}`}>
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        <View className="flex-row items-center justify-between">
          ...
          <TouchableOpacity className="px-6" onPress={() => navigation.navigate('SettingsScreen')}>
        ...
        {!isVerified && (
          ...
          >
            <Image resizeMode="contain" className="h-16 w-16 absolute left-7" source={Images.SHPE_WHITE} />
            ...
            <View className="flex-1">
              ...
              >
                <Text className={`text-xl text-white font-medium`}>
                  {isSignedIn ? 'Office Sign Out' : 'Office Sign In'}
                </Text>
              </TouchableOpacity>
            </View>
          </View>
        ...
        <View className="flex-row flex-1 items-center mx-4 mt-4 space-x-4">
          ...
          <View className="flex-1">
            <TouchableOpacity
              ...
              className={`flex-1 flex-row justify-between items-center h-14 rounded-lg  ${
                officeCount > 0 ? 'bg-primary-blue' : 'bg-grey-dark'
              }`}
              style={{
                shadowColor: '#000',
                shadowOffset: {
            ...
            >
              <Text className={`text-xl text-white font-medium flex-1 text-center`}>
                {officeCount > 0 ? 'Knock on wall' : 'Unavailable'}
              </Text>
              <TouchableOpacity
              ...
              >
                <View>
                  {officeHourInfoMenu && (
                    <View
                      className={`absolute rounded-md z-20 border w-44 right-0 top-12 px-3 py-2  ${
                        darkMode ? 'bg-secondary-bg-dark border-grey-dark' : 'bg-secondary-bg-light border-grey-light'
                      }`}
                    >
                      <Text className={`text-xl font-medium ${darkMode ? 'text-white' : 'text-black'}`}>
                        Notify an officer in Zach 450 for assistance.
                      </Text>
                    </View>
                  )}
        ...
        <View className="mt-12 -z-10">
          <View className="flex-row mb-3">
            <View className="flex-row ml-4 flex-1">
              ...
              <TouchableOpacity className="px-4" onPress={() => setInterestOptionsModal(true)}>
                <Octicons name="gear" size={24} color={darkMode ? 'white' : 'black'} />
              </TouchableOpacity>
            </View>
            <TouchableOpacity
              ...
              onPress={() =>
                navigation
                  .getParent()
                  ?.navigate('EventsTab', { screen: 'EventsScreen', params: { filter: 'myEvents' } })
              }
            >
          ...
          {isLoading && <ActivityIndicator size="small" className="mt-8" />}
          {!isLoading && myEvents.length === 0 && (
            <View
              className={`mx-4 mt-4 p-4 rounded-md flex-row ${
                darkMode ? 'bg-secondary-bg-dark' : 'bg-secondary-bg-light'
              }`}
              style={{
                shadowColor: '#000',
                shadowOffset: {
          ...
          <View className="mx-4">
            {!isLoading &&
              myEvents.map((event: SHPEEvent, index) => {
                return (
      ...
      {officeHourInfoMenu && (
        <TouchableOpacity className="flex-1 h-screen w-screen absolute" onPress={() => setOfficeHourInfoMenu(false)} />
      )}

      <DismissibleModal visible={knockOnWallConfirmMenu} setVisible={setKnockOnWallConfirmMenu}>
        <View
        ...
        >
          ...
          <View className="flex items-center w-[80%] mt-4">
            <Text className={`text-center text-lg font-bold ${darkMode ? 'text-white' : 'text-black'}`}>
              Are you sure you want to notify the signed-in officers?
            </Text>
      ...
      <DismissibleModal visible={signInConfirmMenu} setVisible={setSignInConfirmMenu}>
        <View
          className={`flex opacity-100 rounded-md p-6 ${darkMode ? 'bg-secondary-bg-dark' : 'bg-secondary-bg-light'}`}
        >
          <Octicons name="bell" size={24} color={darkMode ? 'white' : 'black'} />
          ...
          <View className="flex items-center w-[80%] mt-4">
            <Text className={`text-center text-lg font-bold ${darkMode ? 'text-white' : 'text-black'}`}>
              {isSignedIn
                ...
                : 'You will receive notifications from members. Are you sure you want to sign in?'}
            </Text>
      ...
      <DismissibleModal visible={interestOptionsModal} setVisible={setInterestOptionsModal}>
        <View
        ...
        >
          ...
          <View className="flex-row items-center justify-between">
            <View className="flex-row items-center ml-3">
              {darkMode ? (
                <Image resizeMode="cover" className="h-14 w-14" source={Images.SHPE_WHITE} />
              ) : (
                <Image resizeMode="cover" className="h-14 w-14" source={Images.SHPE_NAVY} />
              )}
          ...
          <Text className={`mx-4 text-lg ${darkMode ? 'text-white' : 'text-black'}`}>
            You will receive notifications for events related to your interests and committees
          </Text>
          ...
          <View className="mt-8 mx-4">
            ...
            <Text className={`text-lg ${darkMode ? 'text-white' : 'text-black'}`}>
              Select the type of events you are interested in
            </Text>
            ...
            <View>
              ...
              >
                <View
                  className={`items-center justify-center w-full bg-primary-blue rounded-md px-2 py-2 ${
                    isInterestChanged() ? 'bg-primary-blue' : darkMode ? 'bg-grey-dark' : 'bg-grey-light'
                  }`}
                >
          ...
          <View className="mt-8 mx-4">
            ...
            <Text className={`text-lg ${darkMode ? 'text-white' : 'text-black'}`}>
              Visit committees tab to join or leave a committee
            </Text>
              ...
              <View className="mt-2">
                <View className="flex-row flex-wrap mt-2">
                  {userCommittees?.map((committeeName, index) => {
                    return (
                      <View
                        className={`border py-2 px-3 mr-2 mb-2 rounded-md border-primary-blue ${
                          darkMode ? 'bg-secondary-bg-dark' : 'bg-secondary-bg-light'
                        }`}
                        style={{
                          shadowColor: '#000',
