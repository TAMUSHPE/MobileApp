  ...
  const handleUserAuth = () => {
    ...
    initializeCurrentUserData()
      .then(async (userFromFirebase) => {
        ...
        const isUserInBlacklist = httpsCallable<{ uid: string }, { isInBlacklist: boolean }>(
          functions,
          'isUserInBlacklist'
        );
      ...
      <KeyboardAwareScrollView showsVerticalScrollIndicator={false} className="flex-1">
        <SafeAreaView className="flex-1 h-screen">
          ...
          <View className="mx-8 mt-10">
            ...
            <TouchableOpacity
              className="pr-4"
              onPress={() => navigation.navigate('GuestRecoveryAccount')}
              activeOpacity={1}
            >
            ...
            {loading && <ActivityIndicator className="mt-4" size="small" />}
          </View>
