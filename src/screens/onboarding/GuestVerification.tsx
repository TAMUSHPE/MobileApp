...
const GuestVerification = ({ navigation }: NativeStackScreenProps<AuthStackParams>) => {
  ...
  return (
    <LinearGradient colors={['#191740', '#413CA6']} className="flex-1">
      <SafeAreaView className="flex-1">
        {/* Header */}
        <View className="px-4 mt-5">
          <TouchableOpacity onPress={() => navigation.navigate('LoginScreen')} activeOpacity={1}>
            <Octicons name="chevron-left" size={30} color="white" />
          </TouchableOpacity>
        </View>

        <View className="items-center">
          <Image className="flex-row h-20 w-20 mt-2 mb-14" source={Images.SHPE_LOGO} />
        </View>
        ...
        <View className="mx-8 mt-10 items-center">
          ...
          <Text className="text-white text-center text-lg mt-4">
            We sent an email to <Text className="text-primary-orange text-bold">{auth.currentUser?.email}</Text> {'\n'}
        ...
        <View className="mx-8 flex-1 justify-end">
          {!auth.currentUser?.emailVerified && !resend && (
            <Pressable onPress={resendVerificationEmail}>
              <Text className="text-white text-center text-lg text-semibold">
                Did not receive verification email? <Text className="text-primary-orange">Resend</Text>
              </Text>
            </Pressable>
          )}

          {resend && <Text className="text-primary-orange text-lg">Another verification email has been resend</Text>}
        </View>
      </SafeAreaView>
    </LinearGradient>
  );
