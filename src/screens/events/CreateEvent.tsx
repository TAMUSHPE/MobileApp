...
const CreateEvent = ({ navigation }: NativeStackScreenProps<EventsStackParams>) => {
  ...
  const EventTypeButton = ({
    ...
    return (
      <View className="w-[47%] mt-8">
        <TouchableOpacity
          className={`w-[100%] flex-row px-2 py-4 items-center rounded-lg border ${
            darkMode ? 'border-grey-light bg-secondary-bg-dark' : 'border-grey-dark bg-secondary-bg-light'
          }`}
          style={{
            shadowColor: '#000',
            shadowOffset: {
        ...
        >
          <View
            className={`h-7 w-7 border-2 rounded-full items-center justify-center ${
  ...
  return (
    <SafeAreaView
      edges={['top']}
      className={`flex flex-col h-screen ${darkMode ? 'bg-primary-bg-dark' : 'bg-primary-bg-light'}`}
    >
      ...
      <View className={`flex-1`}>
        ...
        <View className="px-4">
          <View className="flex-row justify-between">
            ...
            <EventTypeButton
              eventType={EventType.COMMITTEE_MEETING}
              label="Committee Meeting"
              Image={CommitteeMeetingIcon}
            />
          </View>
