        ...
        <View className="-z-10">
          ...
          <View className="mx-4 mt-3">
            ...
            <Text className={`text-lg ${darkMode ? 'text-grey-light' : 'text-grey-dark'}`}>
              {eventType}
              {committee && ' • ' + reverseFormattedFirebaseName(committee)} • {calculateMaxPossiblePoints(event)}{' '}
              points
            </Text>
            <Text className={`text-lg ${darkMode ? 'text-grey-light' : 'text-grey-dark'}`}>
          ...
          <View
            className={`mt-6 mx-4 p-4 rounded-lg ${darkMode ? 'bg-secondary-bg-dark' : 'bg-secondary-bg-light'}`}
            style={{
              shadowColor: '#000',
              shadowOffset: {
                width: 0,
            ...
            {locationName && (
              <View className="mt-4">
                ...
                <View className="flex-row flew-wrap items-center">
                  <Text className={`text-xl font-bold ${darkMode ? 'text-white' : 'text-black'}`}>{locationName}</Text>
                  {geolocation && (
                    <TouchableOpacity
                      onPress={() => {
                        if (Platform.OS === 'ios') {
                          handleLinkPress(
                            `http://maps.apple.com/?daddr=${geolocation.latitude},${geolocation.longitude}`
                          );
