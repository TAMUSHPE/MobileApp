...
export const hexToRGBA = (hexColor: string): ColorValues | undefined => {
    ...
    if (hexColor.length % 3 == 0) {
        ...
        const tripletRegex: RegExp = /^#?(\dA-Fa-f)(\dA-Fa-f)(\dA-Fa-f)/i;
        hexColor = hexColor.replace(tripletRegex, (m, r, g, b): string => {
            return "#" + r + r + g + g + b + b;
        });
        ...
        const quadrupletRegex: RegExp = /^#?(\dA-Fa-f)(\dA-Fa-f)(\dA-Fa-f)(\dA-Fa-f)/i;
